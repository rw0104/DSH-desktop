...
[dshfind](https://dshfind.com) is another optional cooperating catalog source. Its reviewed adapter reads the public atomic `/v1/catalog` snapshot only after the user adds and selects it. The Host validates the snapshot's version, timestamp, total, item identities, provenance, and exact origin before publishing the local index; one bounded gzip-capable request avoids the provider's paginated anonymous-rate limit. It is not selected by default, preferred, recommended, or used as a fallback.
