...
The Host completes one full normalized scan for the selected source and current locale before serving catalog interactions. Standard sources are exhausted through their declared cursor and effective page limits. The reviewed 1024Store adapter instead performs one full-registry GET, normalizes every valid item, and emits Schema-bounded chunks of at most 100 items. The dshfind adapter performs one atomic full-catalog GET, validates the response's own `data_version`, total, timestamp, identities, and exact origin, then emits the same bounded chunks locally. Its reviewed Host transport accepts only identity or gzip, caps decoded 1024Store responses at 16 MiB and dshfind responses at 32 MiB, and never extends those exceptions to user-added sources. The 25,000-item normalized Host limit, source identity, cancellation, provenance, and origin checks apply across the whole scan.
