...
[dshfind](https://dshfind.com) is an optional cooperating source with a reviewed built-in adapter for its public REST API. The adapter uses only the compiled-in `https://api.dshfind.com` origin and public anonymous requests. It reads one atomic `GET /v1/catalog` response and validates its `data`, `total`, `data_version`, `as_of`, duplicate identities, and exact final origin before publishing any index. An invalid version, inconsistent total, malformed entry set, cancellation, transport failure, or oversized response rejects the complete scan; no partial index is published. Search, category filtering, category enumeration, sorting, and 50-item UI pagination are then performed against the complete local index without sending those interactions back to dshfind.

The built-in adapter deliberately uses dshfind's documented full-catalog endpoint instead of replaying its paginated listing. This reduces a complete synchronization to one provider request, so catalog growth no longer turns the documented anonymous quota into a multi-minute client-side crawl. The reviewed Host client accepts only identity or gzip encoding, caps the decoded response at 32 MiB, and retains a 25,000-item normalized-index ceiling. A completed local index remains subject to the ordinary bounded cache; explicit refresh starts a new atomic full download.
