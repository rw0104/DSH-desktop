...
- **图片与文件操作**：高级模式输入框的 + 菜单支持添加图片、上传文件到当前工作区和打开指令；支持保存原图及消息、代码、链接的内容菜单。
- **上传后引用**：文件存入当前工作区，路径加入草稿；每次最多 10 个文件、每个不超过 25 MiB，同名文件独立保存。PDF/Office 解析仍取决于工具。
- **Markdown 本地图片**：Sidebar 拆分预览支持相对路径、中文和空格图片引用。
