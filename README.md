...
当前源码版本为 **2.2.3**：接入 pm01 语音动画核心，改进字幕滚动、任务时的语音窗口及供应商模型列表拉取。见 [v2.2.3 更新说明](docs/releases/v2.2.3.md)。图片输入自动识别继续保留，独立生图执行接口尚未接入。Harness `0.1.2-rc.1` 和 Better Sidebar `0.18.0` 保持不变；新版本下载资产发布前，上方链接仍指向已发布的 v2.1.0。
