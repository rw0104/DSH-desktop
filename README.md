...
当前源码版本为 **2.2.2**：修复实时语音按钮点击时读取旧版会话字段导致无响应的问题，保留 2.2.1 的自定义模型图片能力自动识别。见 [v2.2.2 更新说明](docs/releases/v2.2.2.md)及 [v2.2.1 图片能力说明](docs/releases/v2.2.1.md)。Harness `0.1.2-rc.1` 和 Better Sidebar `0.18.0` 保持不变；新版本下载资产发布前，上方链接仍指向已发布的 v2.1.0。
...
安装器已经包含 DSH Runtime 和 Better Sidebar，不要求客户电脑预装 Node.js 才能启动桌面应用。Vision Toolkit 已从 v2.1.0 正式产品和安装器彻底移除。
