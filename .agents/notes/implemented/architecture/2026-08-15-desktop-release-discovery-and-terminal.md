...
No installer endpoint is touched until the user chooses **Download**. The checker first repeats the version request and proceeds only if the same newer version is still advertised. Windows then downloads the exact versioned `Update.exe` asset from this repository's GitHub Release; macOS retains its existing DMG delivery endpoint. Electron `net.fetch` streams at most 1 GiB into a private versioned user-data directory, synchronizes and atomically renames the completed file, and removes partial files after failure or cancellation. The handoff requires the macOS artifact to carry a UDIF `koly` trailer and the Windows artifact to contain DOS and PE signatures. These checks reject an HTML error or structurally wrong artifact but do not prove publisher identity.

macOS opens the validated DMG and explains that the user must replace DSH Desktop in `Applications` and reopen it. It does not mount and mutate the installed signed bundle itself. Windows asks again after the NSIS installer is ready. **Restart and Install** starts the installer with exact argv and no shell, waits for its spawn event, then requests the existing bounded Cordis teardown before the current application exits. Choosing **Later**, or any download, filesystem, or installer-opening error, produces no failure UI and leaves the available-version tray action retryable. Manual check failures use the fixed retry dialog described above.

Release ordering is an operational invariant: the version tag, GitHub Release, versioned Windows Update asset, blockmap, and `latest.yml` must exist before the release is considered discoverable. Publishing the GitHub Release makes that version visible to the checker; drafts and prereleases are not returned by the Latest Release endpoint.
...
Packaged DSH Desktop can discover and download a newer stable release only after explicit confirmation, and it can provide the ordinary desktop-profile plugin workflow without changing the upstream checkout or weakening renderer isolation. macOS replacement remains manual; Windows installation uses the downloaded NSIS program after a second confirmation. The generated CLI environment remains local to terminals opened from the tray.

This repository's GitHub Latest Release is the release-version authority, and the Windows installer URL is bound to that exact version. The desktop package also owns the bundled pnpm version and generated shim behavior, which increases the packaged runtime closure and must remain aligned with Electron's ABI. Linux retains compatibility but has neither an installer-download path nor a desktop terminal until separate platform designs are implemented.
