...
| 原问题 | 实施结果 | 当前边界 |
| --- | --- | --- |
| v2.0.8 更新体验 | 更新器现在从 Release 资产清单取得精确版本、文件名、大小和 SHA-256；下载到应用私有目录，校验通过后才进入重启安装 | v2.0.9 已以 GitHub Latest 部署；本版透明披露为 `NotSigned`，受信 Authenticode/publisher lock 仍是后续安全工作 |
...
| Vision Toolkit 接管图片 | 从正式产品 Bundle、不可变集合、直接依赖、设置入口和安装包移除；旧 Profile 只做清理，不恢复插件 | 原生 `image` 模型永远优先官方附件链；v2.0.9 不再存在第三方输入接管路径 |
...
实现提交及验证记录见 [`2026-08-24_v2.0.9-remediation-task-plan.md`](2026-08-24_v2.0.9-remediation-task-plan.md) 和 [`releases/v2.0.9.md`](releases/v2.0.9.md)。本附录不改变原始调查证据，只标注调查后的产品状态；原始报告中“可选保留”建议已按用户决定切换为正式产品移除分支。
