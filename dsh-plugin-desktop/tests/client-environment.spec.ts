...
describe('advanced desktop layout', () => {
  it('lifts Better Sidebar rail toggles onto the reference toolbar centerline', () => {
    ...
    try {
      ...
      expect(css).toMatch(/editorBody[^}]*:has\([^)]*editorPlaceholder[^)]*\)[^{]*> \[class\*="editorMain"\] \{[^}]*display:\s*none !important;/)
      expect(css).toMatch(/editorBody[^}]*:has\([^)]*editorPlaceholder[^)]*\)[^{]*> \[class\*="editorTreeDock"\] \{[^}]*width:\s*100% !important;/)
      expect(css).toMatch(/editorBody[^}]*:has\([^)]*editorPlaceholder[^)]*\)[^{]*editorTreeResize[^}]*\{[^}]*display:\s*none !important;/)
