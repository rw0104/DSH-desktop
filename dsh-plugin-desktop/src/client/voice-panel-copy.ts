...
const english = {
  ...
  privacy: 'Microphone audio is sent to {provider}, configured separately from your chat model.',
  endHint: 'Ending native voice also interrupts its delegated task.',
  move: 'Move voice controls', moveHelp: 'Drag to move, or use arrow keys. Shift moves faster; Home resets the position.', resetPosition: 'Reset position',
  taskRunning: 'Agent is working', taskCompleted: 'Task completed', taskFailed: 'Task failed', taskCancelled: 'Task interrupted',
  taskApproval: 'Waiting for approval',
...
const chinese: VoicePanelCopy = {
  ...
  privacy: '麦克风音频发送给 {provider}，该语音服务与聊天模型独立配置。',
  endHint: '结束原生语音也会中断它委托的任务。',
  move: '移动语音控件', moveHelp: '拖动或按方向键移动；Shift 加速，Home 恢复默认位置。', resetPosition: '重置位置',
