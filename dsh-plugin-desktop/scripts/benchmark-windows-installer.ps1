...
try {
  ...
  if ($Scenario -eq 'uninstall') {
    $taskProcess = Start-Process -FilePath $taskUninstallerPath -ArgumentList '/S' -PassThru -Wait -WindowStyle Hidden
  } else {
  ...
  if (Test-Path -LiteralPath $taskUninstallerPath) {
    $null = Start-Process -FilePath $taskUninstallerPath -ArgumentList '/S' -PassThru -Wait -WindowStyle Hidden
  }
