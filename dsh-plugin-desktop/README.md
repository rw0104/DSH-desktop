...
The `DSH Desktop Windows Release` GitHub Actions workflow runs the same Windows x64 build on manual dispatch or a pushed `v*` tag. It uploads the NSIS installer, blockmap, and `latest.yml`; tag builds also attach those files to a GitHub Release. The local attachment backend requires `sharp` at runtime, so Windows packages retain `sharp` and the current-architecture `@img/sharp-win32-x64` native entry; Electron Builder must not exclude them.
