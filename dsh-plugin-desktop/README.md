...
Packaged applications query the product-owned GitHub latest-release API 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline and shares one in-flight operation with the **Check for Updates…** tray command. Responses are accepted only for a non-draft, non-prerelease `v<stable-semver>` tag. Background network, HTTP, timeout, invalid-response, equal-version, and older-version outcomes are silent. A manual check always opens a native result dialog. Automatic prompts are remembered per version, while the tray can retry explicitly. Development and unpackaged launches never download an installer; this downstream channel currently enables confirmed downloads only on Windows.

Choosing **Download** first rechecks that the advertised version is unchanged, then opens a native save dialog. The default destination is Downloads, but the user may choose another absolute path and filename; cancelling makes no download request. Windows requests the exact tag-owned `DSH-Desktop-<version>-x64-Setup.exe`, follows GitHub's redirect through Electron networking, streams at most 1 GiB, records the installer for the upgrade handoff, and rejects an incomplete PE. **Restart and Install** launches the NSIS installer and requests orderly Cordis teardown. After the upgraded app starts, it offers to delete or keep the installer. Failures leave the current version intact and keep the tray action retryable. macOS currently reports availability without offering a nonexistent downstream DMG.

Release operators must publish the complete Windows asset set before GitHub marks the tagged Release as latest. The tag, package version, release note filename, and Setup asset version must match exactly. Drafts and prereleases are ignored by the application.
